  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function addItem(title, value, preadded, locked, focusme) {
        ...
        var li = null;
        if (options.enableCardIdLink)
        	li = $('<li class="'+liclass+'" rel="'+value+'" id="pt_'+id+'"></li>').prepend("<a href='javascript:void(0)' onclick='"+getCardIdLinkCall()+"'>"+xssDisplay(title)+"</a>").append(aclose);
        else
        	li = $('<li class="'+liclass+'" rel="'+value+'" id="pt_'+id+'"></li>').prepend(xssDisplay(title)).append(aclose);
      ...
      function addInput(focusme) {
        ...
        input.keyup( function(event) {
          ...
          if (options.searchType == 'word'){
        	  if ( event.keyCode == _key.space ) {
        		  load_feed($.trim(etext)); //Trim foi acrescentado por causa do espaço que é adicionado quando se usa o truque do espaço
                  complete.children(".default").hide();
                  feed.show();
        	  }
          } else if (options.serchType = 'character'){
      ...
      function bindEvents() {
        ...
        feed.children("li").unbind("mousedown").mousedown( function() {
          var option = $(this);
          addItem(option.attr("display"), option.attr("rel"), 0, 0, 1);
          clear_feed(1);
          complete.hide();
        });
      ...
      var _key = { 'enter': 13,
                   ...
                   'apostrof': 96,
                   'space' : 32 //ADICIONEI AQUI O SPACE
                 };
