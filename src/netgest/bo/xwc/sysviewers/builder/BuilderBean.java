package netgest.bo.xwc.sysviewers.builder;
...
public class BuilderBean {
	...
	private String elapsedMs = "Elapsed Time: 00:00:00";
	...
	public String getElapsedTime() {
		if( builderInProgress ) {
		    ...
		    elapsedMs = "Elapsed Time: " + StringUtils.padl( Long.toString( (int)elapsedTimeHour  ), 2, "0") + ":" + 
		...
		if( builderInProgress ) {
			...
			if( buildSucess != null ) {
				if( buildSucess ) {
					...
					XUIRequestContext.getCurrentContext().addMessage("build_sucess", 
							new XUIMessage( XUIMessage.TYPE_ALERT, XUIMessage.SEVERITY_INFO , 
									"XEO Model Builder", 
									"Build was completed Sucessfull"
							)	
						);
				}
				else {
					XUIRequestContext.getCurrentContext().addMessage("build_error", 
							new XUIMessage( XUIMessage.TYPE_ALERT,XUIMessage.SEVERITY_CRITICAL , 
									"XEO Model Builder", 
									"Error building XEO Models, check log for details."
							)	
						);
				}
			}
	...
	public void startBuild() throws Exception {
		...
		try {
    		...
    		Form 		oForm 		  = (Form)viewRoot.findComponent( Form.class );
