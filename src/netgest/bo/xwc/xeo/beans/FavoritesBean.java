...
public class FavoritesBean extends XEOBaseBean {
	...
	public GridColumnRenderer getClassName(){
		...
		public String render(GridPanel grid, DataRecordConnector record,
			...
			boObject toDisplay;
			try {
				toDisplay = boApplication.getDefaultApplication().
					getObjectManager().loadObject(getEboContext(), boui);
				//boDefHandler.getBoDefinition(boName)
	...
	public void removeFavorite(){
		...
		if (selectedBouiForDelete > 0 ){
			try {
				...
				boObject toRemoveObj = obManager
					.loadObject(getEboContext(), selectedBouiForDelete);
				...
				getRequestContext().addMessage("deleted", 
						new XUIMessage(XUIMessage.TYPE_ALERT, XUIMessage.SEVERITY_INFO,
								BeansMessages.FAVORITES_BEAN_DELETED_MSG_TITLE.toString(), 
								BeansMessages.FAVORITES_BEAN_DELETED_MSG.toString()));
				//Reset
			...
			getRequestContext().addMessage("noObject", 
					new XUIMessage(XUIMessage.TYPE_ALERT, XUIMessage.SEVERITY_INFO, 
							BeansMessages.FAVORITES_BEAN_NO_OBJ_MSG_TITLE.toString(), 
							BeansMessages.FAVORITES_BEAN_NO_OBJ_MSG.toString()));
		}
	}
