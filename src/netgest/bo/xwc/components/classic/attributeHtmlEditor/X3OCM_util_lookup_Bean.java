...
public class X3OCM_util_lookup_Bean extends XEOBaseLookupList {
	...
	public X3OCM_util_lookup_Bean() throws boRuntimeException {
		...
		HttpServletRequest request = (HttpServletRequest) oRequestContext
		...
		objsReq = (objsReq != null && !"".equals(objsReq)) ? objsReq
				.replaceAll(",", ";") : "";
		objsReq = (!"".equals(objsReq)) ? objsReq : "Ebo_ClsReg"; // Default
	 ...
	 * 
	 * @see
	 * netgest.bo.xwc.xeo.beans.XEOBaseLookupList#setSelectedObject(java.lang
	 * .String)
	 */
	@Override
	...
	public void confirm() {
		...
		XUIRequestContext oRequestContext = XUIRequestContext
				.getCurrentContext();
		GridPanel oGridComp = (GridPanel) oRequestContext.getViewRoot()
				.findComponent(GridPanel.class);
		DataRecordConnector currDataRec = oGridComp.getActiveRow();

		long currBoui = 0;

		if (currDataRec != null) {
			currBoui = Long.parseLong(oGridComp.getActiveRow().getAttribute(
					"BOUI").getDisplayValue());
			try {
				selectedObj = boObject.getBoManager().loadObject(oEboContext,
						currBoui);
		...
		oRequestContext.getScriptContext().add(
				...
				"function_ret",
				"try{parent.handleXeoRetPopupVal('" + currBoui + "', '"
						+ cmpIdOrigin + "', '" + winIdOrigin + "', '"
						+ obj2BeCalled + "', '" + selectedObj.getName()
	...
	public void addNew() throws Exception {
		...
		XUIViewRoot oEditViewRoot = oRequestContext.getSessionContext()
				.createChildView(
						getViewerResolver().getViewer(sObjectName,
								XEOViewerResolver.ViewerType.EDIT));
	...
	public XEOObjectListConnector getDataList() {
		...
		EboContext oEboContext = boApplication.currentContext().getEboContext();
		boObjectList currentObjectList = boObjectList.list(oEboContext, boql,
				false, false);
	...
	public String getCurrentObj() {
		String currentObj = (getSelectedObject() == null ? currentObject
				: getSelectedObject());
		return currentObj;
	}
	...
	public String getCurrentObjNameNew() {
		String menuText = netgest.bo.xwc.xeo.localization.X3OCM_Mensagens.V_X3OCM_util_lookup_addNew_sPar
				.toString(getCurrentObjName());
		return menuText;
	}
